...
public class DefaultVehicleMarkService
    ...
    public List<VehicleMark> getAll() {
        ...
        log.debug("Getting all VehicleMarks from DAO. Count of objects - {}"
                , vehicleMarks.size());
        return vehicleMarks;
    }
    ...
    public List<VehicleMark> getByCategoryId(Long categoryId) {
        ...
        log.debug("Getting VehicleMarks from DAO filtered by Category id = {}. Count of objects - {}"
                , categoryId
                , result.size());
        return result;
    }
    ...
    public List<VehicleMark> getByCategoryValue(int categoryValue) {
        ...
        log.debug("Getting VehicleMarks from DAO filtered by Category value = {}. Count of objects - {}"
                , categoryValue
                , result.size());
        return result;
    }
    ...
    public List<SimpleDTO> getByCategoryValueAndNameStartsWithDto(int categoryValue, String searchString) {
        List<SimpleDTO> result = MODEL_MAPPER
                .map(vehicleMarkDao.findByCategoryValueAndNameStartsWithIgnoreCase(categoryValue, searchString, SORT_BY_NAME_ASC),
                        CATEGORY_DTO_TYPE);
        log.debug("Getting VehicleMarks from DAO filtered by Category value = {} "
    ...
    public int saveAll(List<VehicleMark> vehicleMarks, Category category) {
        ...
        log.debug("Saving all VehicleMark by Category id = {}. Count of incoming objects - {}"
                , categoryId
                , vehicleMarks.size());
        vehicleMarks.removeAll(getByCategoryId(categoryId));
        log.debug("Saving all VehicleMark by Category id = {}. Count of objects after filtering - {}"
                , categoryId
                , vehicleMarks.size());
